...
const elements = {
  ...
  downloadJson: document.getElementById('download-site-json'),
...
function setApiMode(mode, message = '') {
  state.apiMode = mode;
...
function updateApiStatus() {
  ...
  let text = 'Loading site content…';
  let className = 'admin-status admin-status--info';
  const detail = state.apiMessage ? ` ${state.apiMessage}` : '';

  switch (state.apiMode) {
    case 'live':
      text = 'Connected to the server API. Publishing will sync changes instantly.';
      className = 'admin-status admin-status--success';
      break;
    case 'local':
      text = `Server API unavailable.${detail} Working from the local site.json snapshot.`.trim();
      className = 'admin-status admin-status--warning';
      break;
    case 'blank':
      text = `No site data was found.${detail} Start filling everything in, then publish to download site.json or reconnect the API.`.trim();
      className = 'admin-status admin-status--warning';
      break;
    default:
      break;
  ...
  elements.apiStatus.className = className;

  if (elements.downloadJson) {
    elements.downloadJson.hidden = !state.data;
    elements.downloadJson.textContent = state.apiMode === 'live' ? 'Download backup' : 'Download site.json';
  }
...
function attachEventListeners() {
  ...
  elements.downloadJson?.addEventListener('click', () => {
    if (!state.data) return;
    downloadSiteJson(clone(state.data));
    if (elements.jsonStatus) {
      elements.jsonStatus.textContent = 'Downloaded the current site.json snapshot.';
...
async function loadContent() {
  ...
  try {
    ...
    if (fallback) {
      ...
      setApiMode('local', error?.message || '');
    } else {
      ...
      statusClass = 'admin-status admin-status--warning';
      setApiMode('blank', error?.message || '');
    }
  }
  if (elements.jsonStatus) {
...
function downloadSiteJson(data) {
  if (!data) return;
...
async function saveData(nextData) {
  const offline = state.apiMode === 'local' || state.apiMode === 'blank';
  if (!offline && !state.token) {
  ...
  if (offline) {
    downloadSiteJson(nextData);
    applyData(nextData, { snapshotSource: nextData });
    if (elements.jsonStatus) {
      elements.jsonStatus.textContent = 'Server API unavailable. Downloaded site.json so you can upload it manually.';
      elements.jsonStatus.className = 'admin-status admin-status--warning';
    }
    return;
  }
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404 || response.status === 405) {
        downloadSiteJson(nextData);
        applyData(nextData, { snapshotSource: nextData });
        setApiMode('local', payload.error || `Save failed (${response.status})`);
        if (elements.jsonStatus) {
          elements.jsonStatus.textContent = 'Server rejected the save. Downloaded site.json instead—upload it manually to apply changes.';
          elements.jsonStatus.className = 'admin-status admin-status--warning';
        }
        return;
      }
...
function updatePublishButton() {
  ...
  const pending = state.apiMode === 'unknown';
  const offline = state.apiMode === 'local' || state.apiMode === 'blank';
  const busyLabel = offline ? 'Preparing download…' : 'Publishing…';
  const readyLabel = offline ? 'Download site.json' : state.publishLabel;
  const shouldDisable = state.saving || pending || (!state.dirty && !offline);
  elements.publishChanges.disabled = shouldDisable;
  elements.publishChanges.setAttribute('aria-busy', state.saving ? 'true' : 'false');
  elements.publishChanges.textContent = state.saving ? busyLabel : readyLabel;
  if (offline) {
    elements.publishChanges.title = 'API unavailable. Click to download site.json and upload it manually.';
  } else {
    elements.publishChanges.removeAttribute('title');
  }
}
